...
Here we describe a simplified version of the analysis that is at the core of our paper (doi:10.1038/s41591-019-0729-3). In this example, we show how a dramatic reduction in RNA sequencing depth has little impact on the performance of models that predict disease outcome based on tumor gene expression.
...
In this example, we will use adrenocortical carcinoma (ACC) to demonstrate how a drastic reduction in RNA-seq depth still gives enough information to predict outcome of disease. You can change the cancer type by changing "ACC" here to any of the standard cancer type name abbreviations of TCGA: https://gdc.cancer.gov/resources-tcga-users/tcga-code-tables/tcga-study-abbreviations. 
...
Depending, internet connection and machine configuration, this can take up to several minutes.
...
Next we can subsample our count matrix to simulate a 100-fold reduction in sequencing depth and see how that impacts predictive performance.
...
After splitting and scaling the subsampled data, we can build a model that takes the subsampled gene expressio data as input and aims at predicting the relative risk of death (or recurrence).
...
With the model built on subsampled gene expression data we ca try to predict the relative risk of death of patients in the subsampled test set. Just as for full coverage models, we can test whether the predicted relative riskof death correlates with actual survival using Cox regression if the ph-assumption is not violated.
...
These results demonstrate, with the help of an single example, that a strong reduction on sequencing depth does not strongly impact the performance of models that take RNA-seq data and try to predict disease outcome.
...
In case, this analysis can be run on any cancer type available in TCGA (see second code chunk in the beginning; make sure to pick the correct disease outcome [OS vs PFI]). In addition, any repetition from 1 to 100 can be run to ensure the results are not biased due to unequal sample distributions.